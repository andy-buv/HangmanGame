...
# Runs through the functions in order
	...
	with open('hangmanresultsv2.txt', 'w+') as g: # opens file we will write results to
		# Writes Header
		header = 'Input Words, Guessed Letters, Miss Letters, No. Misses\n'
		...
		for word in dictionary:
			...
			if '-' not in word:
				...
				while solution != answer:
					...
					if tries == 0:
						sub_dictionary = buildSubDict(dictionary,solution_regex)
